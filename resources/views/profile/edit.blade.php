...
<div class="max-w-4xl mx-auto px-6 py-10 animate-fade-in-up">
    ...
    <form method="POST" action="{{ route('profile.update') }}" enctype="multipart/form-data">
        ...
        <div class="form-section">
            <div class="form-section-label">CEP / Endereço</div>
            <div class="space-y-3">
                <input type="text" id="profile_cep" name="cep" value="{{ old('cep', $user->musicianProfile?->cep) }}" placeholder="CEP" class="input-subtle" maxlength="9">
                <input type="text" id="profile_address" name="address" value="{{ old('address', $user->musicianProfile?->address) }}" placeholder="Endereço" class="input-subtle">
            </div>
        </div>
        ...
        <div class="form-section">
            ...
            <div class="space-y-3">
                <select id="profile_state" name="state" class="input-subtle">
                    <option value="">Selecione ...</option>
                    @foreach(['AC','AL','AP','AM','BA','CE','DF','ES','GO','MA','MT','MS','MG','PA','PB','PR','PE','PI','RJ','RN','RS','RO','RR','SC','SP','SE','TO'] as $uf)
                        <option value="{{ $uf }}" {{ old('state', $user->musicianProfile?->state) === $uf ? 'selected' : '' }}>{{ $uf }}</option>
                    @endforeach
                </select>
                <input type="text" id="profile_city" name="city" value="{{ old('city', $user->musicianProfile?->city) }}" placeholder="Cidade" class="input-subtle">
            </div>
        </div>
        ...
        <div class="form-section">
            <div class="form-section-label">CEP / Endereço / Número</div>
            <div class="space-y-3">
                <input type="text" id="profile_cep" name="cep" value="{{ old('cep', $user->establishmentProfile?->cep) }}" placeholder="CEP" class="input-subtle" maxlength="9">
                <input type="text" id="profile_address" name="address" value="{{ old('address', $user->establishmentProfile?->address) }}" placeholder="Endereço" class="input-subtle">
                <input type="text" name="number" value="{{ old('number', $user->establishmentProfile?->number) }}" placeholder="Número" class="input-subtle" style="max-width:150px;">
            </div>
        </div>
        ...
        <div class="form-section">
            ...
            <div class="space-y-3">
                ...
                <input type="text" id="profile_city" name="city" value="{{ old('city', $user->establishmentProfile?->city) }}" placeholder="Cidade" class="input-subtle">
            </div>
        </div>
        @endif
        ...
        {{-- Social Links --}}
        <div class="form-section">
            <div class="form-section-label">Links Externos</div>
            <div class="space-y-3">
                <div class="flex items-center gap-2">
                    <span class="w-8 text-center" style="color:#E1306C;">&#xf16d;</span>
                    <input type="url" name="social_instagram" value="{{ old('social_instagram', $user->social_links['instagram'] ?? '') }}" placeholder="https://instagram.com/seu-perfil" class="input-subtle">
                </div>
                <div class="flex items-center gap-2">
                    <span class="w-8 text-center" style="color:#1DB954;">&#x266B;</span>
                    <input type="url" name="social_spotify" value="{{ old('social_spotify', $user->social_links['spotify'] ?? '') }}" placeholder="https://open.spotify.com/artist/..." class="input-subtle">
                </div>
                <div class="flex items-center gap-2">
                    <span class="w-8 text-center" style="color:#FF0000;">&#x25B6;</span>
                    <input type="url" name="social_youtube" value="{{ old('social_youtube', $user->social_links['youtube'] ?? '') }}" placeholder="https://youtube.com/@seu-canal" class="input-subtle">
                </div>
                <div class="flex items-center gap-2">
                    <span class="w-8 text-center" style="color:#F59E0B;">&#x1F310;</span>
                    <input type="url" name="social_website" value="{{ old('social_website', $user->social_links['website'] ?? '') }}" placeholder="https://seusite.com" class="input-subtle">
                </div>
                <div class="flex items-center gap-2">
                    <span class="w-8 text-center" style="color:#43E660;">&#x1F517;</span>
                    <input type="url" name="social_linktree" value="{{ old('social_linktree', $user->social_links['linktree'] ?? '') }}" placeholder="https://linktr.ee/seu-perfil" class="input-subtle">
                </div>
                <div class="flex items-center gap-2">
                    <span class="w-8 text-center" style="color:#9CA3AF;">+</span>
                    <input type="url" name="social_other" value="{{ old('social_other', $user->social_links['other'] ?? '') }}" placeholder="https://outro-link.com" class="input-subtle">
                </div>
            </div>
        </div>
...
const cepField = document.getElementById('profile_cep');
if (cepField) {
    ...
    cepField.addEventListener('blur', function() {
        ...
        fetch(`https://viacep.com.br/ws/${cep}/json/`)
            ...
            .then(data => {
                if (data.erro) return;
                const addr = document.getElementById('profile_address');
                const state = document.getElementById('profile_state');
                const city = document.getElementById('profile_city');
                if (addr && data.logradouro) addr.value = data.logradouro;
